import { products, outfits, stores, styleTags, scenes } from "./data.js";
import { loadState, saveState, toggleInList, addUnique } from "./state.js";
import { el, mount, escapeHtml, on } from "./ui.js";
            ...
            <div class="subtle" style="padding:12px;margin-top:12px;">
              ...
              <div class="row" style="flex-wrap:wrap;margin-top:10px;gap:8px;">
                ${outfits[0].items
                  ...
                  .filter(Boolean)
                  .map((p) => `<span class="chip">${escapeHtml(p.name.split("（")[0])}</span>`)
                  .join("")}
              </div>
        ...
        <div class="row between">
          ...
          <div class="row" style="flex-wrap:wrap;">
            ${styleTags
              ...
              .map(
                (t) =>
                  `<span class="pill ${t.key==="潮流"?"lime":""}" data-tag="${escapeHtml(t.key)}" data-active="${state.profile.tags.includes(t.key)}"><span class="dot"></span>${escapeHtml(t.key)}</span>`
              )
              .join("")}
          </div>
        </div>
...
function OutfitsPage() {
  const { params } = qs("outfits") || { params: new URLSearchParams() };
        ...
        <div class="layout">
          <aside class="panel">
            <div class="group">
              <h3>场景</h3>
              <div class="row" style="flex-wrap:wrap;">
                ${scenes.map((s) => `<span class="pill" data-scene="${escapeHtml(s)}" data-active="${s===scene}"><span class="dot"></span>${escapeHtml(s)}</span>`).join("")}
              </div>
            </div>
            <div class="group">
              <h3>风格标签</h3>
              <div class="row" style="flex-wrap:wrap;">
                ${styleTags.map((t) => `<span class="pill ${t.key==="潮流"?"lime":""}" data-tag="${escapeHtml(t.key)}" data-active="${state.profile.tags.includes(t.key)}"><span class="dot"></span>${escapeHtml(t.key)}</span>`).join("")}
              </div>
              <div class="mini" style="margin-top:8px;">只在“可行动/可解锁/已选中”时用高对比点缀。</div>
            </div>
            <div class="group">
              <h3>预算（≤ ${money(state.profile.budget)}）</h3>
              <input id="budget" type="range" min="200" max="800" step="10" value="${state.profile.budget}" style="width:100%;" />
            ...
            <div class="group">
              ...
              <div class="subtle" style="padding:12px;">
                ...
                <div class="row" style="margin-top:10px;">
                  <span class="chip hot">进度：${Math.min(3, Math.floor((state.liked.length + state.bag.length + state.compare.length) / 3))}/3</span>
          ...
          <main>
            ${q ? `<div class="subtle" style="padding:12px 14px;margin-bottom:14px;"><b>搜索</b>：${escapeHtml(q)}（原型：匹配标题/标签）</div>` : ""}
  ...
  const filtered = outfits
    ...
    .filter((o) => !q || (o.title + " " + o.tags.join(" ")).toLowerCase().includes(q.toLowerCase()))
          ...
          <div class="post" style="border:0;border-radius:0;">
            ...
            <div class="body">
              <div class="row between" style="align-items:flex-start;gap:16px;">
                <div>
                  <h2 style="margin:0 0 8px 0;font-size:22px;">${escapeHtml(o.title)}</h2>
                  <div class="meta">
                    <span class="chip hot">${escapeHtml(o.scene)}</span>
                ...
                <div class="card pad" style="box-shadow:none;">
                  ...
                  <div class="section" style="margin:14px 0 0 0;">
                    ...
                    <div class="cards2" style="margin-top:10px;">
                      ${outfits.filter(x => x.id !== o.id).slice(0,2).map(x => `
                        <div class="card pad" style="box-shadow:none;" data-open="${escapeHtml(x.id)}">
                          <div style="font-weight:900;">${escapeHtml(x.title)}</div>
                          <div class="muted" style="font-size:12px;margin-top:8px;">${escapeHtml(x.scene)} · 预算 ≤ ${money(x.budget)}</div>
                        </div>
                      `).join("")}
                    </div>
                  </div>
    ...
    const line = el(`
      <label class="subtle" style="display:flex;gap:10px;align-items:flex-start;padding:10px 12px;margin-bottom:10px;cursor:pointer;">
        ...
        <div style="flex:1;">
          ...
          <div class="muted" style="font-size:12px;margin-top:4px;">${money(p.price)} · ${p.stock.store ? "门店可试穿" : "仅线上"} · <a href="#/product?id=${encodeURIComponent(p.id)}" class="muted" style="text-decoration:underline;">看商品</a></div>
        </div>
      </label>
    `);
...
function CategoryPage() {
        ...
        <div class="layout" style="margin-top:14px;">
          <aside class="panel">
            ...
            <div class="group">
              <h3>价格</h3>
              <div class="row" style="flex-wrap:wrap;">
                ${["0-99","100-199","200-299","300+"].map(r => `<span class="pill" data-price="${r}" data-active="false"><span class="dot"></span>${r}</span>`).join("")}
              </div>
            </div>
            <div class="group">
              <h3>门店有货</h3>
  ...
  function renderGrid() {
    ...
    if (!list.length) plp.appendChild(el(`<div class="card pad" style="grid-column: 1/-1;"><b>没有匹配的单品</b><div class="muted" style="margin-top:8px;">试试切换风格或关闭门店筛选。</div></div>`));
  }
  renderGrid();
  ...
  const node = el(`
    <div class="card" style="overflow:hidden;">
      ...
      <div style="padding:12px;">
        ...
        ${showStore && p.stock.store ? `<div class="muted" style="font-size:12px;margin-top:10px;">附近可试穿：${escapeHtml(stores[0].name.split("｜")[1] || stores[0].name)}</div>` : ""}
      </div>
    </div>
  `);
...
function ProductPage(id) {
    ...
    <div class="wrap">
      <div class="section">
        ...
        <div class="twoCol" style="margin-top:12px;">
          <div class="gallery">
            <div class="thumbs" id="thumbs">
              ${Array.from({ length: thumbCount }).map((_, i) => `<div class="thumb" data-i="${i}" data-active="${i===0}"></div>`).join("")}
            </div>
            <div class="stage">
              <div class="tag">
        ...
        <div class="section">
          ...
          <div class="cards3">
            ${["165/50｜甜酷上班","158/45｜周末出街","170/55｜开学学院潮"].map(t => `<div class="card pad"><b>${escapeHtml(t)}</b><div class="muted" style="margin-top:10px;">“这件真的很显比例。”（原型文案）</div></div>`).join("")}
          </div>
        </div>
  ...
  p.colors.forEach((c) => {
    const b = el(`<span class="pill" data-c="${escapeHtml(c)}" data-active="${c===color}"><span class="dot"></span>${escapeHtml(c)}</span>`);
  ...
  p.sizes.forEach((s) => {
    const b = el(`<span class="pill" data-s="${escapeHtml(s)}" data-active="${s===size}"><span class="dot"></span>${escapeHtml(s)}</span>`);
    b.addEventListener("click", () => {
      size = s;
    ...
    .forEach((o) => {
      const c = el(`
        <div class="card pad" data-open="${escapeHtml(o.id)}">
          ...
          <div class="muted" style="font-size:12px;margin-top:8px;">${escapeHtml(o.scene)} · 预算 ≤ ${money(o.budget)}</div>
          <div class="row" style="flex-wrap:wrap;margin-top:10px;gap:8px;">
...
function RankingPage() {
  const { params } = qs("ranking") || { params: new URLSearchParams() };
  ...
  const node = el(`
    <div class="wrap">
      <div class="section">
        ...
        <div class="row" style="flex-wrap:wrap;">
          ${tabs.map(([k, label]) => `<a class="pill" href="#/ranking?tab=${encodeURIComponent(k)}" data-active="${k===tab}"><span class="dot"></span>${escapeHtml(label)}</a>`).join("")}
        </div>
        ...
        <div class="subtle" style="padding:12px 14px;margin-top:14px;">
          <div class="row between">
            <div>
              <b>为什么上榜</b>
              <div class="muted" style="font-size:12px;margin-top:4px;">${escapeHtml(explain.join(" · "))}</div>
            </div>
  ...
  ranked.forEach((p) => {
    const card = el(`
      <div class="card pad">
        ...
        <div class="muted" style="font-size:12px;margin-top:10px;">上榜理由：${escapeHtml(explain[0])}</div>
      </div>
    `);
...
function ComparePage() {
  ...
  const node = el(`
    <div class="wrap">
      <div class="section">
        ...
        <div class="grid" style="grid-template-columns: repeat(${Math.max(1, picked.length)}, 1fr); gap:14px; margin-top:14px;" id="cols"></div>
      </div>
    </div>
  `);
  ...
  if (!picked.length) {
    ...
    cols.appendChild(el(`<div class="card pad"><b>还没有选择对比单品</b><div class="muted" style="margin-top:8px;">去分类页点“+对比”。</div></div>`));
    return pageShell("category", node);
  }
  picked.forEach((p) => {
    cols.appendChild(
